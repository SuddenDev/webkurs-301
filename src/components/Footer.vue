...
<template>
  <footer class="w-full mt-18 pt-6 pb-8 max-w-3xl text-sm flex flex-col sm:(flex-row justify-between) gap-4 border-main border-t !border-op-50 text-dark dark:text-white">
    ...
    <div flex>
      <span op-70>&copy;&nbsp;&nbsp;{{ new Date().getFullYear() }}&nbsp;
        <a :href="siteConfig.authorUrl" target="_blank" class="nav-link">{{ siteConfig.author
        }}</a></span>
    </div>
  </footer>
</template>
